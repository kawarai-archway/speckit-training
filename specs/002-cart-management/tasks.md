...
- [ ] T001 [P] Extend CartSchema with tax and total fields in src/contracts/cart.ts
- [ ] T002 [P] Add stock field to ProductFetcher interface in src/contracts/cart.ts
- [ ] T003 [P] Add quantity max constraint (99) to cart input schemas in src/contracts/cart.ts
...
- [ ] T004 [P] Create cart usecases implementation in src/domains/cart/api/usecases.ts
- [ ] T005 [P] Create CartView component in src/domains/cart/ui/CartView.tsx
- [ ] T006 Update cart domain API exports in src/domains/cart/api/index.ts
- [ ] T007 Update cart domain UI exports in src/domains/cart/ui/index.tsx
- [ ] T008 [P] Update buyer layout to enable cart link in src/app/(buyer)/layout.tsx
- [ ] T009 Update cart page with data fetching and event handling in src/app/(buyer)/cart/page.tsx
...
- [ ] T010 [P] [US1] Create add to cart usecase unit test in tests/unit/domains/cart/usecase.test.ts
- [ ] T011 [P] [US1] Create product detail page UI unit test in tests/unit/domains/cart/ui.test.tsx
- [ ] T012 [P] [US1] Create cart API integration test in tests/integration/domains/cart/api.test.ts
- [ ] T013 [P] [US1] Create add to cart E2E test in tests/e2e/cart-buyer-flow.spec.ts
...
- [ ] T014 [P] [US1] Implement addToCart usecase with stock validation in src/domains/cart/api/usecases.ts
- [ ] T015 [P] [US1] Implement duplicate prevention logic in addToCart usecase in src/domains/cart/api/usecases.ts
- [ ] T016 [US1] Update product detail page API URL and add feedback in src/app/(buyer)/catalog/[id]/page.tsx
- [ ] T017 [US1] Add loading state and error handling to add-to-cart button in src/app/(buyer)/catalog/[id]/page.tsx
- [ ] T018 [US1] Implement cart-updated event dispatch in src/app/(buyer)/catalog/[id]/page.tsx
- [ ] T019 [US1] 全テスト実行・パス確認（Red テストが全てパスすることを検証）
...
- [ ] T022 [P] [US2] Create cart display usecase unit test in tests/unit/domains/cart/usecase.test.ts
- [ ] T023 [P] [US2] Create CartView component unit test in tests/unit/domains/cart/ui.test.tsx
- [ ] T024 [P] [US2] Create cart GET API integration test in tests/integration/domains/cart/api.test.ts
- [ ] T025 [P] [US2] Create cart view E2E test in tests/e2e/cart-buyer-flow.spec.ts
...
- [ ] T026 [P] [US2] Implement getCart usecase with tax calculation in src/domains/cart/api/usecases.ts
- [ ] T027 [P] [US2] Implement CartView component with product list display in src/domains/cart/ui/CartView.tsx
- [ ] T028 [P] [US2] Implement empty cart state in CartView component in src/domains/cart/ui/CartView.tsx
- [ ] T029 [US2] Add tax calculation logic (10%, floor) to cart usecases in src/domains/cart/api/usecases.ts
- [ ] T030 [US2] 全テスト実行・パス確認（Red テストが全てパスすることを検証）
...
- [ ] T044 [P] [US4] Create remove cart item usecase unit test in tests/unit/domains/cart/usecase.test.ts
- [ ] T045 [P] [US4] Create delete confirmation dialog unit test in tests/unit/domains/cart/ui.test.tsx
- [ ] T046 [P] [US4] Create cart item delete API integration test in tests/integration/domains/cart/api.test.ts
- [ ] T047 [P] [US4] Create item deletion E2E test in tests/e2e/cart-buyer-flow.spec.ts
...
- [ ] T048 [P] [US4] Implement removeCartItem usecase in src/domains/cart/api/usecases.ts
- [ ] T049 [P] [US4] Add delete buttons to CartView component in src/domains/cart/ui/CartView.tsx
- [ ] T050 [US4] Integrate ConfirmDialog from templates for delete confirmation in src/domains/cart/ui/CartView.tsx
- [ ] T051 [US4] Handle empty cart state after last item deletion in src/domains/cart/ui/CartView.tsx
- [ ] T052 [US4] 全テスト実行・パス確認（Red テストが全てパスすることを検証）

### Refactor: 改善

> 重複排除・命名改善・責務分離。全テストパスを検証する。

- [ ] T053 [US4] リファクタリングと全テストパス確認

### 検証: E2Eテスト実行 + カバレッジ確認

> E2E 実行証跡 + カバレッジ 80% 以上確認 + 外部 URL 検証

- [ ] T054 [US4] E2E テスト実行（証跡付き）+ カバレッジ確認
...
- [ ] T075 [P] Update navigation links and cartUrl in buyer layout in src/app/(buyer)/layout.tsx
- [ ] T076 [P] Add comprehensive error boundaries and loading states across cart components
- [ ] T077 [P] Optimize performance for cart operations (debouncing, optimistic updates)
- [ ] T078 [P] サンプルテストリグレッション確認（npm run test:samples）
- [ ] T079 [P] Run quickstart.md validation with updated cart functionality
- [ ] T080 Ensure all cart operations follow security best practices
