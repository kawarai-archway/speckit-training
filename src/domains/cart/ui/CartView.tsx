...
function CartItemRow({
  ...
  const [optimisticQuantity, setOptimisticQuantity] = React.useState(item.quantity);
  const debounceRef = useRef<NodeJS.Timeout | null>(null);

  // Reset optimistic quantity when item quantity changes (from server)
  React.useEffect(() => {
  ...
  return (
    <>
      <div className="flex items-center gap-4 border-b border-base-900/10 py-4" data-testid="cart-item">
        {/* 商品画像 */}
        {item.imageUrl ? (
          <img
            src={item.imageUrl}
        ...
        <div className="flex-1">
          <h3 className="font-medium text-base-900" data-testid="item-name">{item.productName}</h3>
          <p className="text-lg font-bold text-base-900" data-testid="item-price">{formatPrice(item.price)}</p>
        </div>
        ...
        {/* 小計 - Use optimistic quantity for immediate feedback */}
        <div className="w-24 text-right">
          <p className="font-bold text-base-900">{formatPrice(item.price * optimisticQuantity)}</p>
        </div>
      ...
      <div className="rounded-lg border border-base-900/10 bg-white p-6">
        ...
        <div className="mt-6 space-y-2 border-t border-base-900/10 pt-6">
          ...
          <div className="flex justify-between">
            <span className="text-sm text-base-900/60">商品数</span>
            <span className="text-sm text-base-900/60">{cart.itemCount}点</span>
          </div>

          {/* 商品合計 */}
          <div className="flex justify-between">
            <span className="text-base text-base-900">商品合計</span>
            <span data-testid="cart-subtotal" className="text-base font-medium text-base-900">
          ...
          {/* 消費税 */}
          <div className="flex justify-between">
            <span className="text-base text-base-900">消費税（10%）</span>
            <span data-testid="cart-tax" className="text-base font-medium text-base-900">
          ...
          <div className="flex justify-between border-t border-base-900/10 pt-2">
            <span className="text-lg font-bold text-base-900">総合計</span>
            <span data-testid="cart-total" className="text-xl font-bold text-base-900">
